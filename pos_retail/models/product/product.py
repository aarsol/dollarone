...
class product_template(models.Model):
	...
	is_prop = fields.Boolean('Contain Prop.')
	pos_prop_ids = fields.One2many('pos.product.prop', 'product_tmpl_id', string='Product Prop Notes')

	variant_categ = fields.Boolean('Variant Category')
	variant_categ_id = fields.Many2one('pos.category','Variant Category')
	variant_product_id = fields.Many2one('product.product','Variant Default Product')

	cross_selling = fields.Boolean('Cross selling')
	cross_ids = fields.One2many('product.cross', 'product_tmpl_id', 'Cross selling')

	supplier_barcode = fields.Char('Supplier Barcode', copy=False, help="Barcode of product from supplier.")
	...
	min_stock_qty = fields.Integer('Minimum Required Qty')
	max_stock_qty = fields.Integer('Maximum Required Qty')
