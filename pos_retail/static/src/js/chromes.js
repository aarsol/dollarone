odoo.define('pos_retail.chromes', function (require) {
    ...
    chrome.Chrome.include({
        build_widgets: function () {
            ...
            if (this.pos.config.screen_type && this.pos.config.screen_type == 'waiter'){                
                $('.rightpane').addClass('waiter');
            }
        }
    });
    ...
    var CountItemWidget = chrome.StatusWidget.extend({
        ...
        start: function () {
            var self = this;
            if (self.pos.config.screen_type && self.pos.config.screen_type == 'waiter')
                $('.leftpane').css({'left': '586px'});
            ...
            $('.show_hide_buttons').click(function () { // do not add this.$
                if (self.show) {
                    ...
                    if (self.pos.config.screen_type && self.pos.config.screen_type == 'waiter') {
                        $('.buttons_pane').animate({width: 146}, 'slow');
                        $('.leftpane').animate({left: 146}, 'slow');
                        $('.rightpane').animate({left: 586}, 'slow');
                    } else {
